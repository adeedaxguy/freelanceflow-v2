...
const editableSetup = (value: CallingSetup) => JSON.stringify({ provider: value.provider || "elevenlabs", profile: value.profile, voiceId: value.voiceId, phoneId: value.phoneId });
...
export default function AdminCallingClient() {
  ...
  const providerName = setup.provider === "retell" ? "Retell" : "ElevenLabs";
  ...
  async function saveSetup() {
    const ok = await act("Saving setup", { action: "setup", provider: setup.provider || "elevenlabs", profile: setup.profile, voiceId: setup.voiceId, phoneId: setup.phoneId, ...(apiKey ? { apiKey } : {}) }, true);
    if (ok) { setApiKey(""); setNotice(`Saved. Load ${providerName} voices, select a voice and outgoing number, then verify the agent.`); }
  }
  ...
  return <div className="mx-auto w-full max-w-[1440px] px-4 pb-12 pt-20 sm:px-8 lg:pt-8">
    ...
    {snapshot && tab === "setup" && <fieldset disabled={Boolean(activeRun)} aria-label="Agent configuration" className="grid min-w-0 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.6fr)]">
      ...
      <div className="space-y-5 border-t border-border pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
        ...
        <label className="block space-y-1.5 text-sm">Voice provider<select className={inputClass} value={setup.provider || "elevenlabs"} disabled={Boolean(busy)} onChange={e => { setSetup({ ...setup, provider: e.target.value as "retell" | "elevenlabs", voiceId: "", connected: false, ready: false, phoneId: setup.phoneId.startsWith("workspace:") ? setup.phoneId : "" }); setApiKey(""); setOptions({ numbers: [], voices: [] }); }}><option value="retell">Retell</option><option value="elevenlabs">ElevenLabs</option></select></label>
        {setup.provider === "retell" && <p className="text-xs leading-relaxed text-muted-foreground">Trial credits are limited; Twilio calls and SIP usage cost extra. Use the Retell API key marked for webhooks. No card or automatic top-up is needed for the trial.</p>}
        <label className="block space-y-1.5 text-sm">{providerName} API key<input type="password" autoComplete="new-password" className={inputClass} placeholder={setup.connected ? "Connected; leave blank to keep" : "Enter the provider API key"} value={apiKey} onChange={e => setApiKey(e.target.value)} /></label>
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">Your existing softphone number keeps its incoming calls and manual calling. No number import is needed. Voice, model and Twilio usage are billed separately.</p>
        <button className={primaryClass} disabled={Boolean(busy || runId || apiKey) || !setup.connected || !setup.profile.approved || !setup.voiceId || !setup.phoneId} onClick={async () => { if (await act("Saving agent settings", { action: "setup", provider: setup.provider || "elevenlabs", profile: setup.profile, voiceId: setup.voiceId, phoneId: setup.phoneId }, true)) { if (await act("Verifying voice agent", { action: "provision" }, true)) setNotice("Agent configured. Test on an owned, consenting number before a business campaign."); } }}><ShieldCheck size={16} /> Verify agent</button>
        <div className="border-t border-border pt-5 text-sm"><p className="font-medium">Pilot limits</p><ul className="mt-2 space-y-2 text-muted-foreground"><li>1 concurrent call · 3 minutes per call</li><li>10 calls / 30 reserved minutes per 24 hours</li><li>Weekdays, 10am–4pm recipient local time</li><li>{setup.provider === "retell" ? "Written notes only in iCloseLeads; Retell audio is temporary and disclosed to callers" : "Written notes; audio recording disabled"}</li><li>Meeting requests saved for human follow-up</li><li>Calendar booking and SMS not connected</li></ul></div>
      </div>
    </fieldset>}
