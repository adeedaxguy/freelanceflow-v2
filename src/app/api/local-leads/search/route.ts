...
export async function POST(req: NextRequest) {
  ...
  let foursquareKey     = process.env.FOURSQUARE_API_KEY      ?? "";
  ...
  foursquareKey = pick("foursquareKey",     foursquareKey);
  ...
  try {
    ...
    for (const s of settings) {
      ...
      if (s.key === "foursquare_api_key")  foursquareKey     = s.value;
  ...
  const result = await searchLocalBusinesses({
    ...
    // France SIRENE + OpenCorporates run automatically (no key needed)
    db:                prisma,
  });
