...
Live key connection and voice selection passed after deployment. The owner subsequently approved the Lofts Studio brief, and that approval was saved through the admin form. Initial creation succeeded but verification blocked readiness because Retell omits cleared null fields and the empty JSON-schema required array. Its exported configuration reproduced the mismatch. The narrow fix accepts omitted nullable fields and omits the unnecessary empty required array; privacy, call limits, prompts and tools remain mandatory. A verification failure now retains returned agent IDs so a retry updates the same agent. The real exported configuration passes the corrected comparison. Type-check and all 508 tests pass, including omitted critical-field rejection and retry-without-duplicate-creation coverage. Live verification will be repeated after this fix deploys and reported in the task.

Mock tests do not establish real SIP audio quality, latency, interruptions, caller-ID display or successful end-to-end telephone delivery. A consenting owned-number pilot is still required before business calling. Retell credits do not cover Twilio PSTN/SIP usage. Retell temporary audio processing is disclosed; iCloseLeads stores written notes only. No telephone calls have been placed during setup.
