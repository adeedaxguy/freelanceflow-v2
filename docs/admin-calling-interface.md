...
1. Select your existing admin softphone number, which is available before connecting a voice provider. Choose Retell or ElevenLabs, save its key, load female voices, approve Lofts Studio facts/offer limits and verify the agent. Already-connected dedicated numbers remain an ElevenLabs alternative. Switching providers resets voice/readiness while preserving an existing workspace number.
...
- **Unknown provider recovery:** dispatching/uncertain attempts expose conversation-ID reconciliation for the exact attempt. Controlled hold release requires at least 15 minutes, evidence and confirmation that neither the voice provider nor Twilio has an active call; release keeps the campaign paused and never automatically retries the number. Retell registrations with an unknown Twilio dispatch remain reviewable only after Retell is terminal too.
...
- Agent configured is setup state, not a completed phone-call test. The administrator's existing number can be reused without changing incoming routing. Every dial remains gated by consent, fresh research, suppression, local-time checks and pilot limits: one concurrent call, three minutes, ten attempts per rolling 24 hours and seven-day duplicate exclusion.
- Retell trial credits are finite, with separate Twilio/SIP usage costs. The key must be the webhook-signing key. Retell stores basic call metadata; signed callbacks deliver written notes to iCloseLeads. Temporary Retell audio processing/recording is disclosed in both setup and the greeting; no audio or recording URL is retained by iCloseLeads. Missing notes hold the run for review.
- Calendar booking, SMS, unattended scheduling and automatic CRM promotion are unavailable. Twilio recording stays disabled. Operator review of applicable calling/recording obligations remains required; this surface does not certify legal compliance.
